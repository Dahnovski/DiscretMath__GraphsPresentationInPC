...
int main()
 {
    ...
    int choice;
    printf("Вас приветствует программа по преобразованию различных форм хранения графа.\n\n");
    printf("Что от меня требуется?\n");
...
void MIin() // ввод матрицы инцидентности
{
    ...
    printf("M = "); // количество дуг
    scanf("%d", &M);
    printf("N = "); // количество вершин
    scanf("%d", &N);
    A=(int *)malloc(M*N*sizeof(int)); // выделение памяти для вводимой матрицы инцидентности
    readArray(A, M, N);
    printf("-----------------------------------");
    printf("\nВаша матрица инцидентности:\n");
    printArray(A, M, N);
    printf("----------------------------------\n");
...
void readArray(int *A, int M, int N) // ввод матрицы
{
    int i, j;
    for(i = 0; i < M; i++)
       for(j = 0; j < N; j++)
...
node *readList() // ввод списка смежности
{
    int i = 0, semn;
    int sizeOfNode=sizeof(node);
    node *A = NULL, *R1 = NULL, *R2;
    printf("%d :  ", ++i);
    scanf("%d",&semn);
    if(semn)
    {
...
void printArray(int *A, int M, int N) // вывод матрицы
{
   int i, j;
   for(i = 0; i < M; i++)
    {
...
void printList(node *R) // вывод списка смежности
{
    while(R)
    {
        printf("%2d", R->value);
...
void MI_MS(int *A, int *B, int M, int N) // матрица инцидентности в список смежности
{
    int i, j, a = 0, b = 0;
    for(i = 0; i < M; i++)
    {
...
void MS_SS(int *A, int N, node **arrayOfList) // Матрица смежности в список смежности
{
    int i, j, k;
    int sizeOfNode=sizeof(node);
    node *R1, *R2;
    ...
    for(i = 0; i < N; i++)   //найти  первую 1 в строке
    {
        ...
        //k=0 - нет в строке единицы, иначе k - номер первой 1 в строке (отсчёт с 1)
        if(k)
        {
            R1=(node *)malloc(sizeOfNode);
...
void SS_MI(int *A, int N, node **arrayOfList) // список смежности в матрицу инцидентности
{
    int i, k = 0;
    node *R;
    for(i = 0; i < N; i++)
...
int countInList(node **arrayOfList, int N) //подсчёт элементов списка смежности
{
    int i, k=0;
    node *R;
    for(i=0; i<N; i++)
...
void destroy(node *R) //освобождение памяти для строки из списка
{
   node *TMP;
   while(R)
    {
